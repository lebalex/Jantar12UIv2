...
public class ExcelAdapter implements ActionListener {
    ...
    public static final Logger logger_job = Logger.getLogger(ExcelAdapter.class);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().compareTo("Copy") == 0) {
            ...
            if (!((numrows - 1 == rowsselected[rowsselected.length - 1]
                ...
                JOptionPane.showMessageDialog(null, "Ошибка копирования!",
                        "Ошибка копирования!", JOptionPane.ERROR_MESSAGE);
                return;
            }
        ...
        if (e.getActionCommand().compareTo("Cut") == 0) {
            ...
            for (int i = 0; i < numrows; i++) {
                for (int j = 0; j < numcols; j++) {
                    if(jTable1.isCellEditable(rowsselected[i],colsselected[j]))
        ...
        if (e.getActionCommand().compareTo("Paste") == 0) {
            ...
            try {
                ...
                for (int i = 0; st1.hasMoreTokens(); i++) {
                    ...
                    String sep = "\t";
                    //if(rowstring.indexOf("\t")>=0)sep = "\t";
                    StringTokenizer st2 = new StringTokenizer(rowstring, sep);
                    for (int j = 0; st2.hasMoreTokens(); j++) {
                        value = (String) st2.nextToken().trim();
